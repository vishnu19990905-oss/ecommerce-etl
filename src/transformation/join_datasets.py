...
def join_datasets(df_Customers,
    ...
    final_df = final_df.select(
        ...
        (col("oi.price").cast('int').alias("price")),
        (col("oi.shipping_charges").cast('int').alias("shipping_charges")),
        (col("oi.item_total_amount").cast('int').alias("item_total_amount")),
        col("p.payment_type"),
        (col("p.payment_value").cast('int').alias("payment_value")),
